...
SOURCE_TABLE = "TRIPJACK_SPLIT9"
TARGET_TABLE = "TRIPJACK_SPLIT10"
...
WHERE AirportIATACode2 = AirportIATACode4
AND AirportIATACode5 IS NULL
"""
...
def find_all_split_points(flights, airports):
    """
    Rule 1 — Exactly 3 flights, 4 airports, Airport2 == Airport4:
        e.g. MLE → CMB → DXB → CMB
             [0]    [1]   [2]   [3]
        Split at flight boundary 2:
          Segment 1: Flight1,Flight2 | Airport1 → Airport2 → Airport3       (MLE → CMB → DXB)
          Segment 2: Flight3 | Airport3 → Airport4       (DXB → CMB)
