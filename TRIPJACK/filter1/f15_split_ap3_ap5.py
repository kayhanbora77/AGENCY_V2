...
SOURCE_TABLE = "TRIPJACK_SPLIT4"
TARGET_TABLE = "TRIPJACK_SPLIT5"
...
WHERE AirportIATACode3 = AirportIATACode5
...
# COLUMN LISTS
# ============================================================================
PREFIX_FLIGHT = "FlightNumber"
PREFIX_DATE = "DepartureDateLocal"
PREFIX_AIRPORT = "AirportIATACode"

FLIGHT_COLS = [f"{PREFIX_FLIGHT}{i + 1}" for i in range(MAX_FLIGHTS)]
DATE_COLS = [f"{PREFIX_DATE}{i + 1}" for i in range(MAX_DATES)]
AIRPORT_COLS = [f"{PREFIX_AIRPORT}{i + 1}" for i in range(MAX_AIRPORTS)]
...
def get_flights_airports(row_list):
    ...
    for i in range(MAX_FLIGHTS):
        fn = row_list[COL_IDX[f"{PREFIX_FLIGHT}{i + 1}"]]
...
def find_all_split_points(flights, airports):
    """
    Rule 1 — Exactly 5 flights, 6 airports, Airport3 == Airport5:
        e.g. MAA → DEL → FRA → TIA → FRA → YYZ
             [0]    [1]   [2]   [3]   [4]   [5]
        Split at flight boundary 1:
          Segment 1: Flight1,Flight2,Flight3 | Airport1 → Airport2 → Airport3 → Airport4       (MAA → DEL → FRA → TIA)
          Segment 2: Flight4,Flight5       | Airport4 → Airport5 → Airport6       (TIA → FRA → YYZ)
...
def build_child_list(parent_list, all_cols, flights_slice, airports_slice, parent_id):
    ...
    for i, (fn, fd) in enumerate(flights_slice):
        child[COL_IDX[f"{PREFIX_FLIGHT}{i + 1}"]] = fn
        child[COL_IDX[f"{PREFIX_DATE}{i + 1}"]] = fd

    for i, ap in enumerate(airports_slice):
        child[COL_IDX[f"{PREFIX_AIRPORT}{i + 1}"]] = ap
