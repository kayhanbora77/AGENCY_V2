"""
TRIPJACK - Split TRIPJACK_SPLIT7 into TRIPJACK_SPLIT8 (clean) and TRIPJACK_REJECT
(rows where AirportIATACode1 = AirportIATACode4), WITHOUT modifying TRIPJACK_SPLIT7.

Rule:
  If AirportIATACode1 = AirportIATACode4 (and both are non-null):
    - Insert the row into TRIPJACK_REJECT with RejectionReason='Airport1 == Airport4'
  Otherwise:
    - Insert the row into TRIPJACK_SPLIT8 (clean rows)

  TRIPJACK_SPLIT7 is only read from - never deleted or updated.

NOTE - bug fixed here:
  Plain `AirportIATACode1 = AirportIATACode4` evaluates to SQL NULL (neither true
...
SOURCE_TABLE = "TRIPJACK_SPLIT7"
TARGET_TABLE = "TRIPJACK_SPLIT8"
REJECT_TABLE = "TRIPJACK_REJECT"

REJECT_CONDITION = (
    "AirportIATACode1 IS NOT NULL "
    "AND AirportIATACode4 IS NOT NULL "
    "AND AirportIATACode1 = AirportIATACode4"
)
REJECT_REASON = "Airport1 == Airport4"


def split_by_airport1_equals_airport4(db_path: str = DB_PATH) -> None:
...
if __name__ == "__main__":
    split_by_airport1_equals_airport4()