...
TR_SRB_CARRIERS = TR_CARRIERS | SRB_CARRIERS
TR_SERBIA_TIMELIMITS=2.0
...
# DATA LOADING
# ────────────────────────────────────────────────
def eu_airports(con: duckdb.DuckDBPyConnection) -> frozenset:
    log("Loading EU airports...")
    rows = con.execute("""
...
def eueligible_data(con: duckdb.DuckDBPyConnection) -> Optional[pd.DataFrame]:
...
# BUSINESS LOGIC
# ────────────────────────────────────────────────
def calculate_timelimits_vectorized(
    df: pd.DataFrame,
    eu_airports_set: frozenset,
) -> pd.DataFrame:
    ...
    # ── HIGH PRIORITY RULE (row-level flag, evaluated BEFORE aggregation) ──
    # LegNo == 1 AND DepartureAirport (FromAirport) not in eu_airports()
    # AND AirlineCode in TR_CARRIERS or SRB_CARRIERS
    # -> force TimeLimitL1 = 2, TimeLimitL2 = 2 for the whole connection group
    from_airport_norm = df["FromAirport"].astype(str).str.strip().str.upper()
    df["_high_priority_override"] = (
        (df["LegNo"] == 1)
        & (~from_airport_norm.isin(eu_airports_set))
        & (df["AirlineCode"].isin(TR_SRB_CARRIERS))
    )
    ...
    agg = (
        ...
        .agg(
            ...
            depL2=("depL2", "max"),
            arrL2=("arrL2", "max"),
            HighPriorityOverride=("_high_priority_override", "any"),
        )
        .reset_index()
    )
    ...
    # ── Apply high-priority override LAST — takes precedence over everything above ──
    if agg["HighPriorityOverride"].any():
        agg.loc[agg["HighPriorityOverride"], "limitL1"] = TR_SERBIA_TIMELIMITS
        agg.loc[agg["HighPriorityOverride"], "limitL2"] = TR_SERBIA_TIMELIMITS

    # Calculate year difference from Aug 2026
