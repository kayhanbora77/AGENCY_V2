...
SOURCE_TABLE = "AKBAR_SPLIT7_V2"  # input  table (read-only)
TARGET_TABLE = "AKBAR_CLEANED_V2"  # output table for clean / passing rows
REJECTION_TABLE = "AKBAR_REJECTION_V2"  # output table for rejected rows
...
def _normalize_flightno(fn: str) -> str:
    ...
    """
    m = re.fullmatch(r"([A-Z]{1,3})(\d+)", fn.upper().strip())
    if not m:
        return fn
...
def process_table(
    ...
    while True:
        ...
        if not reject_df.empty:
            ...
            # Get actual rejection table column order from DB
            rej_table_cols = col_names(con, rejection_table)
            for c in rej_table_cols:
                if c not in reject_df.columns:
                    reject_df[c] = None
            reject_df = reject_df[rej_table_cols]
            actual_rej_col_list = ", ".join(f'"{c}"' for c in rej_table_cols)
            cursor_rej = con.cursor()
            cursor_rej.execute(
                f'INSERT INTO "{rejection_table}" ({actual_rej_col_list}) SELECT * FROM reject_df'
            )
