...
FN5_FN6_WHERE_CLAUSE = """
WHERE (FLIGHTNUMBER5 IS NOT NULL OR FLIGHTNUMBER6 IS NOT NULL)
"""
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE):
    ...
    # ── Step 1: Insert rows that do NOT match FN5 is NULL AND FN6 is NULL into TARGET ────
    print(f"  Step 1: Copying non-matching rows from '{table}' into '{TARGET_TABLE}'...")
    con.execute(f"""
        INSERT INTO "{TARGET_TABLE}" ({col_list})
        SELECT {col_list} FROM "{table}"
        WHERE FLIGHTNUMBER5 IS NULL AND FLIGHTNUMBER6 IS NULL
    """)
    ...
    con.execute(f"""
        ...
        SELECT {col_list}, 'FN5OrFN6Exist' AS "RejectionReason" FROM "{table}" 
        {FN5_FN6_WHERE_CLAUSE}
    """)
