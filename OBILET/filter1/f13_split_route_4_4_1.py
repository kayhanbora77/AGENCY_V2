...
SOURCE_TABLE = "OBILET_SPLIT2"
TARGET_TABLE = "OBILET_SPLIT3"
...
# Condition to identify exactly 4-flight round-trip rows (where Airport2 == Airport4)
FOUR_FLIGHT_CONDITION = """
    ...
    AND AirportIataCode2 = AirportIataCode4
"""
...
def find_all_split_points(flights, airports):
    ...
    Rule 1 — Exactly 4 flights, 5 airports, Airport2 == Airport4:
        e.g. ELQ → RUH → GIZ → RUH → ELQ
             [0]    [1]   [2]   [3]     [4]
        Split at flight boundary 1:
          Segment 1: Flight1,Flight2         | Airport1 → Airport2 → Airport3  (ELQ → RUH → GIZ)
          Segment 2: Flight3,Flight4         | Airport3 → Airport4 → Airport5  (GIZ → RUH → ELQ)
