...
SIX_FLIGHT_CONDITION = """
    FlightNumber1 IS NOT NULL AND FlightNumber1 <> ''
    AND DepartureDateLocal1 IS NOT NULL

    AND FlightNumber2 IS NOT NULL AND FlightNumber2 <> ''
    AND DepartureDateLocal2 IS NOT NULL

    AND (FlightNumber3 IS NOT NULL AND FlightNumber3 <> '')
    AND DepartureDateLocal3 IS NOT NULL

    AND (FlightNumber4 IS NOT NULL AND FlightNumber4 <> '')
    AND DepartureDateLocal4 IS NOT NULL

    AND (FlightNumber5 IS NOT NULL AND FlightNumber5 <> '')
    AND DepartureDateLocal5 IS NOT NULL

    AND (FlightNumber6 IS NOT NULL AND FlightNumber6 <> '')
    AND DepartureDateLocal6 IS NOT NULL   
    AND AirportIataCode1 IS NOT NULL
    AND AirportIataCode2 IS NOT NULL
    AND AirportIataCode3 IS NOT NULL
    AND AirportIataCode4 IS NOT NULL
    AND AirportIataCode5 IS NOT NULL
    AND AirportIataCode6 IS NOT NULL
    AND AirportIataCode7 IS NOT NULL
    AND AirportIataCode1 = AirportIataCode7
"""
...
AIRPORT_COLS = [f"AirportIataCode{i + 1}" for i in range(MAX_AIRPORTS)]
...
def find_all_split_points(flights, airports):
    """
    Rule 1 — Exactly 6 flights, 7 airports, Airport1 == Airport7:
        e.g. ELQ → RUH → GIZ → XYZ → GIZ → RUH → ELQ
             [0]    [1]   [2]   [3]   [4]     [5]    [6]
        Split at flight boundary 3:
          Segment 1: Flight1,Flight2,Flight3         | Airport1 → Airport2 → Airport3 → Airport4 (ELQ → RUH → GIZ → XYZ)
          Segment 2: Flight4,Flight5,Flight6         | Airport4 → Airport5 → Airport6 → Airport7 (XYZ → GIZ → RUH → ELQ)
    ...
    # ── Rule 1: exactly 6 flights, 7 airports, Airport1 == Airport7 ──────────
    if n_f == 6 and n_a == 7 and airports[0] == airports[6]:
        split_points.add(3)
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE, batch_size=BATCH_SIZE):
    ...
    # ── Step 2: Fetch and process matching 6-flight round-trip rows ───────────
    filtered_total = con.execute(f"""
        SELECT COUNT(*)
        FROM "{table}"
        WHERE {SIX_FLIGHT_CONDITION};
    """).fetchone()[0]
    print(
        f"\n  Step 2: Processing {filtered_total:,} 6-flight round-trip rows for split..."
    )
