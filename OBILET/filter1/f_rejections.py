...
class Reason(str, Enum):
    ...
    MISSING_REQUIRED_SLOT = "MISSING REQUIRED SLOT (FlightNumber1/FlightDate1)"
...
def check_flightno_validity(row) -> tuple[bool, str | None]:
    ...
    for i in range(1, MAX_FLIGHTS + 1):
        fn, dt = row.get(f"FlightNumber{i}"), row.get(f"DepartureDateLocal{i}")
...
def check_missing_required_slot(row) -> tuple[bool, str | None]:
    if _isna(row.get("FlightNumber1")) and _isna(row.get("DepartureDateLocal1")):
        return (
            True,
