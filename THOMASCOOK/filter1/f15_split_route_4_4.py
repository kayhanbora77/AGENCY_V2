...
# SQL condition to identify 4-flight round-trips (BLR -> FRA -> VIE -> FRA -> BLR)
WHERE_4_FLIGHTS = """
    AIRPORT2 = AIRPORT4 AND ((FLIGHTNO4 IS NOT NULL) AND (FLIGHTNO5 IS NULL))
"""
...
def find_all_split_points(flights, airports):
    ...
    Rule 1 — Exactly 4 flights, 5 airports, Airport2 == Airport4:
        e.g. BLR → FRA → VIE → FRA->BLR
             [0]    [1]   [2]   [3]   [4]
        Split at flight boundary 1:
          Segment 1: Flight1, Flight2      | Airport1 → Airport2-> Airport3 (BLR → FRA → VIE)
          Segment 2: Flight3, Flight4      | Airport3 → Airport4 ->Airport5 (VIE → FRA ->BLR)
    """
    n_f = len(flights)
    n_a = len(airports)
