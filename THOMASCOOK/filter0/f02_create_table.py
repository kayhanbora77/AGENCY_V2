...
def create_table():
    ...
    max_lengths = con.execute(f"""
        ...
        SELECT max(f_len), max(a_len), max(d_len), max(arr_len) FROM cleaned_lengths;
    """).fetchone()
    ...
    max_arrival_dates = (
        max_lengths[3] if max_lengths and max_lengths[3] is not None else 1
    )

    print(
        f"Detected Max Counts -> Flights: {max_flights}, De-duplicated Airports: {max_airports}, Departure Dates: {max_dates}, Arrival Dates: {max_arrival_dates}"
    )
