...
SOURCE_TABLE = "TBO_SPLIT"  # input  table (read-only)
...
def process_batch(
    ...
    Returns:
        ...
        reject_df  — rows to write to TBO_REJECTION
...
def check_route_overflow(row) -> tuple[bool, str | None]:
    """Rule 2 — count dates only for slots that have a FlightNumber."""
    fn_count = 0
    for i in range(1, MAX_FLIGHTS + 1):
        fn = row.get(f"FlightNumber{i}")
        if not _isna(fn):
            ...
            break  # stop at first empty FlightNumber slot (slots are contiguous)

    dt_count = 0
    for i in range(1, fn_count + 1):  # only count dates up to fn_count
        dt = row.get(f"DepartureDateLocal{i}")
        if not _isna(dt):
            dt_count += 1
