...
def find_all_split_points(flights, airports):
    """
    Rule 1 — Exactly 3 flights, 4 airports, Airport1 == Airport4:
        e.g. YYC → DEL → DXB → YYC
             [0]    [1]   [2]   [3]
        Split at flight boundary 1:
          Segment 1: Flight1,Flight2 | Airport1 → Airport2 → Airport3           (YYC → DEL → DXB)
          Segment 2: Flight3 | Airport3 → Airport4            (DXB → YYC)
    ...
    # ── Rule 1: exactly 3 flights, 4 airports, Airport1 == Airport4 ──────────
    if n_f == 3 and n_a == 4 and airports[0] == airports[3]:
        split_points.add(2)
