...
AP2_AP5_WHERE_CLAUSE = """
WHERE AIRPORT2 = AIRPORT5
  AND AIRPORT6 IS NULL
"""
...
def find_all_split_points(flights, airports):
    """
    Rule 1 — Exactly 4 flights, 5 airports, Airport2 == Airport5:
        e.g. DEL → MUC → AGP → MXP → MUC
             [0]    [1]   [2]    [3]    [4]
        Split at flight boundary 1:
          Segment 1: Flight1 | Airport1 → Airport2          (DEL → MUC)
          Segment 2: Flight2, Flight3, Flight4 | Airport2 → Airport3 → Airport4 → Airport5          (MUC → AGP → MXP → MUC)
    ...
    # ── Rule 1: exactly 4 flights, 5 airports, Airport2 == Airport5 ──────────
    if n_f == 4 and n_a == 5 and airports[1] == airports[4]:
        split_points.add(1)
