...
SOURCE_TABLE = "TBO_SPLIT5"
TARGET_TABLE = "TBO_SPLIT6"
...
WHERE 
    ...
    AND Airport7 is not null
    AND Airport1=Airport7
"""
...
def find_all_split_points(flights, airports):
    ...
    Rule 1 — Exactly 6 flights, 7 airports, Airport1 == Airport7:
        e.g. BOM → DXB → JNB → GBE → JNB → DXB → BOM
             [0]    [1]   [2]   [3]   [4]   [5]   [6]
        Split at flight boundary 1:
          Segment 1: Flight1,Flight2,Flight3 | Airport1 → Airport2 → Airport3 → Airport4           (BOM → DXB → JNB → GBE)
          Segment 2: Flight4,Flight5,Flight6 | Airport4 → Airport5 → Airport6→ Airport7          (GBE → JNB → DXB → BOM)
    ...
    # ── Rule 1: exactly 6 flights, 7 airports, Airport1 == Airport6 ──────────
