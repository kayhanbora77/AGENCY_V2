...
SOURCE_TABLE = "TBO_SPLIT4"
TARGET_TABLE = "TBO_SPLIT5"
...
WHERE 
    ...
    AND Airport6 is not null
    AND Airport1=Airport6
"""
...
def find_all_split_points(flights, airports):
    """
    Rule 1 — Exactly 5 flights, 6 airports, Airport1 == Airport6:
        e.g. DEL → SIN → SEA → SFO → SIN → DEL
             [0]    [1]   [2]   [3]   [4]   [5]
        Split at flight boundary 1:
          Segment 1: Flight1,Flight2 | Airport1 → Airport2 → Airport3           (DEL → SIN → SEA)
          Segment 2: Flight3,Flight4,Flight5 | Airport3 → Airport4 → Airport5→ Airport6  (SEA → SFO → SIN → DEL)
    ...
    # ── Rule 1: exactly 5 flights, 6 airports, Airport1 == Airport6 ──────────
    if n_f == 5 and n_a == 6 and airports[0] == airports[5]:
        split_points.add(3)
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE, batch_size=BATCH_SIZE):
    ...
    """).fetchone()[0]
    print(
        f"\n  Step 2: Processing {filtered_total:,} 5-flight round-trip rows for split..."
    )
