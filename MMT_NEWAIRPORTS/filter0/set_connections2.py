...
# full connections, EXCEPT when doing so would revisit an airport already on
# the path -- that's the signature of a round trip closing the loop, which
# must stay as separate connections rather than one long chain.
# ============================================================================


def finalize_chain(cluster, indices):
    conn_id = uuid.uuid4()
    for leg_no, idx in enumerate(indices, start=1):
        cluster[idx]["ConnectionId"] = conn_id
        cluster[idx]["LegNo"] = leg_no
    if len(indices) == 1:
        # Never connected to anything else, in either direction -- flag it
        # the same way the original full-graph isolated-component case did.
...
def stitch_segments(cluster, segments):
    ...
    for seg in segments:
        ...
        if bridges and not closes_loop:
            ...
            finalize_chain(cluster, open_chain["indices"])
            open_chain = {
                "indices": list(seg["order"]),
                "first_airport": seg["first_airport"],
                "last_airport": seg["last_airport"],
