...
SOURCE_TABLE = "MIDDLEEAST_SPLIT3"
TARGET_TABLE = "MIDDLEEAST_SPLIT4"
...
def find_all_split_points(flights, airports):
    """
    Rule — Date gap > 1 day between consecutive flights:
        Split at that flight boundary.
        Applies to rows with exactly 3 flights (FlightNo1, FlightNo2, FlightNo3).
    """
    n_f = len(flights)

    # Bug fix: was n_f < 3 which skipped ALL 3-flight rows with no gap;
    # correct guard is n_f < 2 — a single flight can never be split
    if n_f < 2:
        return []
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE, batch_size=BATCH_SIZE):
    ...
    # ── Step 2: Fetch and process exactly-3-flight rows ───────────────────────
    filtered_total = con.execute(f"""
        SELECT COUNT(*)
        FROM "{table}"
        WHERE (FlightNo1   IS NOT NULL AND FlightNo1   <> '')
    ...
    """).fetchone()[0]
    print(
        f"\n  Step 2: Processing {filtered_total:,} exactly-3-flight rows for date-gap split..."
    )
